...
public class registration_click extends AppCompatActivity {
    Button signupbackbutton;
    FirebaseAuth mAuth=FirebaseAuth.getInstance();
    FirebaseFirestore db=FirebaseFirestore.getInstance();
    RadioButton male,female;
    String gender;
    TextInputLayout first_name1, last_name1, phone_no1, emailaddress1, username1, password1, re_password1,address,phone_no2,phone_no3,hobbies;
    ...
    private boolean validatephon2()
    {
        String phone= phone_no2.getEditText().getText().toString().trim();


        if (phone.isEmpty()) {
            phone_no2.setError("Field Can't Be Empty");
            return false;
        } else if (phone.length() !=10) {
            phone_no2.setError("Phone No Should Be Exactly 10Digits ");
            return false;
        }
        else {
            phone_no2.setError(null);
            phone_no2.setErrorEnabled(false);
            return true; }
    }
    private boolean validatephon3()
    {
        String phone= phone_no3.getEditText().getText().toString().trim();


        if (phone.isEmpty()) {
            phone_no3.setError("Field Can't Be Empty");
            return false;
        } else if (phone.length() !=10) {
            phone_no3.setError("Phone No Should Be Exactly 10Digits ");
            return false;
        }
        else {
            phone_no3.setError(null);
            phone_no3.setErrorEnabled(false);
            return true; }
    }
    ...
    private  boolean validatehobbies(){
        String hobbies1 = hobbies.getEditText().getText().toString().trim();
        if (hobbies1.isEmpty()) {
            hobbies.setError("Field Can't Be Empty");
            return false;
        } else {
            hobbies.setError(null);
            hobbies.setErrorEnabled(false);
            return true; }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        phone_no2=findViewById(R.id.phone_no2);
        phone_no3=findViewById(R.id.phone_no3);
        male=findViewById(R.id.male);
        female=findViewById(R.id.female);
        hobbies=findViewById(R.id.hobbies_regisration);

        signupbackbutton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (!validatefirstname() | !validateLastName() | !validateemail() | !validateusername() | !validatepassword() | !validatephone() | !validatephon2() | !validatephon3() | !validatehobbies()) {
                    return;
                }
                ...
                mAuth.createUserWithEmailAndPassword(email2, password2).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            String email2 = emailaddress1.getEditText().getText().toString().trim();
                            String phone1 = phone_no1.getEditText().getText().toString().trim();
                            String username2 = username1.getEditText().getText().toString();
                            String password2 = password1.getEditText().getText().toString().trim();
                            String phone2=phone_no2.getEditText().getText().toString().trim();
                            String phone3=phone_no3.getEditText().getText().toString().trim();
                            String hobbies2=hobbies.getEditText().getText().toString().trim();
                            if(female.isChecked()){
                                gender="Female";
                            }else{
                                gender="Male";
